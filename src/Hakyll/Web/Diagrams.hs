...
diagramsTransformer pandoc = unsafeCompiler $ putStrLn "diagramsTransformer" >> renderBlockDiagrams pandoc
...
renderDiagram shouldPad decls expr (_ident, _cls, fields) = do
  ...
  where
    ...
    sz        = mkSizeSpec2D (Just 200) (Just 200)
...
renderBlockDiagram defs c@(CodeBlock attr@(_, cls, _) s)
    ...
    | ("dia"     `elem` classTags) || ("[dia]"     `elem` classTags) = do
    ...
    | otherwise = do
       putStrLn $ "renderBlockDiagram" ++ (show classTags)
       return c
  where
    (tag, src)        = unTag (traceIt "s:" s)
    classTags         = traceIt "classTags:" ((maybe id (:) tag) (traceIt "cls:" cls))
...
unTag s = either (const (Nothing, s)) id $ parse tag "" (traceIt "unTag:" s)
